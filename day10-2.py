...
path = {}
path[0] = 1 # first adapter has voltage 1

for a in data:
    poss = 0 # ibilities
    for n in range(1, 4):
        poss += path.get(a - n, 0) # add number to possibilities if we have the adapter, else 0
        path[a] = poss
        solution = poss
        #print(poss)

print("posibilities for adapters: ", solution)
